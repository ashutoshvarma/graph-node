...
use fail::fail_point;
use graph::blockchain::block_stream::stream_to_channel;
use graph::blockchain::{BlockchainKind, DataSource};
use graph::data::store::scalar::Bytes;
use graph::data::subgraph::{UnifiedMappingApiVersion, MAX_SPEC_VERSION};
use graph::ext::futures::Canceler;
use graph::prelude::tokio::sync::mpsc::{self, Sender};
...
type SharedInstanceKeepAliveMap =
    Arc<RwLock<HashMap<DeploymentId, Arc<dyn Canceler + Send + Sync>>>>;
...
async fn new_block_stream<C: Blockchain>(
    ...
    filter: C::TriggerFilter,
    sender: Sender<Result<BlockStreamEvent<BlockWithTriggers<C>>, CancelableError<Error>>>,
    block_stream_metrics: Arc<BlockStreamMetrics>,
) -> Result<(), Error> {
    let chain = inputs.chain.clone();

    let block_stream = match chain.is_firehose_supported() {
        true => {
    ...
    stream_to_channel(block_stream, guard, sender.clone()).await
}
    ...
    C: Blockchain,
{
    ...
    loop {
        ...
        let block_stream_canceler = Arc::new(SharedCancelGuard::new());
        let block_stream_cancel_handle = block_stream_canceler.handle();

        let (sender, mut receiver) = mpsc::channel(10);

        let canceller = block_stream_canceler.clone();
        let metrics = ctx.block_stream_metrics.clone();
        let filter = ctx.state.filter.clone();
        let stream_inputs = inputs.clone();
        graph::spawn(new_block_stream(
            stream_inputs,
            canceller,
            filter,
            sender,
        ...
        ctx.state
            ...
            .write()
            .unwrap()
            .insert(inputs.deployment.id, block_stream_canceler.clone());

        debug!(logger, "Starting block stream");

        // Process events from the stream as long as no restart is needed
        loop {
            let (block, cursor) = match receiver.recv().await {
